...
export class ImportProductsService {
    ...
    async exec(products: Product[]): Promise<void> {
        ...
        for (const product of products) {
            if (this.wasIdentifierProcessedBefore(product.props.identifier, seenIDentifiers)) {
                this.logger.warn({ message: 'Product identified as duplicated: identifier', identifier: product.props.identifier });
                await this.queueProvider.publishToDLQ({ data: product.props }, new DuplicatedIdentifierError());
            } else {
                const created = await this.createProduct({ ...product.props, imageUrl: undefined });
                if (created) {
    ...
    private async createProduct(product: ProductProps): Promise<Product | undefined> {
        try {
            const created = await this.productRepository.create(product);
            this.logger.log({ message: 'Product created', product: created?.props });
            return created;
        } catch (error) {
            await this.queueProvider.publishToDLQ({ data: product }, new CreateProductError());
    ...
    private async publishToProcessQueue(product: ProductProps) {
        ...
        this.logger.log({ message: 'Product sent to process image queue', product });
    }

    private wasIdentifierProcessedBefore(identifier: string, seenIdentifiers: Set<string>) {
