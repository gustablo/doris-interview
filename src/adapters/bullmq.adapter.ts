...
export class BullMQAdapter implements QueueProvider {
  ...
  async publish(props: PublishProps): Promise<void> {
    try {
      ...
      this.logger.log({
        message: 'Product sent to product-processor queue',
        product: props.data,
      });
    } catch (error) {
      this.logger.error({
        message: 'Error publishing to product-processor queue',
        error,
      });
    }
  }
  ...
  async publishToDLQ(props: PublishProps, reason: BaseError): Promise<void> {
    try {
      ...
      this.logger.log({
        message: 'Product sent to product-processor-dlq queue',
        product: props.data,
      });
    } catch (error) {
      this.logger.error({
        message: 'Error publishing to product-processor-dlq queue',
        error,
      });
    }
  }
